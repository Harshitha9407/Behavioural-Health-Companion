...
class _MentalHealthJournalScreenState extends State<MentalHealthJournalScreen> with TickerProviderStateMixin {
  ...
  // This function now uses mock local data instead of a database
  void _fetchJournalEntries() {
    setState(() {
      _journalEntries = [
        {
          'id': _uuid.v4(),
          'content': 'Had a tough day at work but a nice dinner with my family helped me de-stress.',
          'moodRating': 6,
          'stressRating': 7,
          'emotions': 'Tired,Frustrated,Grateful',
          'moodTags': 'Work,Family',
          'timestamp': DateTime.now().subtract(const Duration(hours: 3)),
        },
        {
          'id': _uuid.v4(),
          'content': 'Feeling much better after a morning run. The fresh air and exercise always helps clear my head.',
          'moodRating': 8,
          'stressRating': 3,
          'emotions': 'Happy,Motivated,Relaxed',
          'moodTags': 'Health,Exercise',
          'timestamp': DateTime.now().subtract(const Duration(days: 1)),
        },
        {
          'id': _uuid.v4(),
          'content': 'Spent the day working on a personal project. It was challenging, but very fulfilling. I feel confident and focused.',
          'moodRating': 9,
          'stressRating': 2,
          'emotions': 'Confident,Peaceful,Focused',
          'moodTags': 'Personal Growth',
  ...
  List<Map<String, dynamic>> get _filteredEntries {
    return _journalEntries.where((entry) {
      final matchesSearch = _searchQuery.isEmpty || 
          ...
          entry['emotions'].toString().toLowerCase().contains(_searchQuery.toLowerCase()) ||
          entry['moodTags'].toString().toLowerCase().contains(_searchQuery.toLowerCase());
  ...
  void _saveEntry() {
    ...
    final newEntry = {
      ...
      'moodTags': _selectedMoodTags.join(','),
      ...
      child: Row(
        ...
        children: [
          ...
          ElevatedButton(
            ...
            child: Text(_currentStep == 2 ? 'Complete Check-in' : 'Continue'),
          ),
        ],
      ),
    );
  ...
  Widget _buildStatsOverview() {
    ...
    final avgMood = recentEntries.map((e) => (e['moodRating'] ?? 0) as int).fold(0, (a, b) => a + b) / recentEntries.length;
    final avgStress = recentEntries.map((e) => (e['stressRating'] ?? 0) as int).fold(0, (a, b) => a + b) / recentEntries.length;
  ...
  Widget _buildDetailedEntryCard(Map<String, dynamic> entry) {
    String moodEmoji = _moodEmojis[entry['moodRating']] ?? '❓';
    String emotions = entry['emotions'] is String && entry['emotions'].isNotEmpty
        ? entry['emotions']
        : 'Not specified';
    String moodTags = entry['moodTags'] is String && entry['moodTags'].isNotEmpty
        ? entry['moodTags']
        : 'Not specified';
  ...
  Widget _buildInfoGrid(Map<String, dynamic> entry) {
    return Row(
      children: [
        Expanded(
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 8),
          const Text('Track your thoughts, emotions, and wellbeing journey',
